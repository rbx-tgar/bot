...
pub mod xp {
    ...
    pub async fn add(
        ctx: Context<'_>,
        #[description = "The usernames of the people you wish to add xp to"] users: Vec<String>,
        #[description = "The amount of XP you want to add"] amount: u64,
    ) -> Result<(), Error> {
        for id in collect_user_ids(&users).await {
            tgar_bot_mongodb::add_xp(mongo_client().await, id as i64, amount as i64).await?;
        }
        ctx.reply(format!("Successfully added {} XP to {} user(s)!", amount, users.len())).await?;
        Ok(())
    }
    ...
    pub async fn remove(
        ctx: Context<'_>,
        #[description = "The usernames of the people you wish to take xp from"] users: Vec<String>,
        ...
        ctx.reply(format!("Successfully removed {} XP from {} user(s)!", amount, users.len()))
            .await?;
        Ok(())
    }
    ...
    pub async fn set(
        ctx: Context<'_>,
        #[description = "The usernames of the people you wish to set the XP of"] users: Vec<String>,
        ...
        ctx.reply(format!("Successfully set {} users XP to {}!", users.len(), amount)).await?;
        Ok(())
    }
}
